...
// Import file storage if available
let fileStorage;
try {
  fileStorage = require('./storage/fileStorage');
} catch (error) {
  console.log('File storage module not available');
}
...
app.post('/api/forms/:id/submit', async (req, res) => {
  try {
    ...
    if (dbConnected && pool) {
      ...
      formExists = inMemoryStorage.forms.some(form => form.id === id);
    }
...
app.get('/api/forms/:id/submissions', async (req, res) => {
  try {
    ...
    if (dbConnected && pool) {
      ...
      const [formRows] = await pool.query('SELECT id FROM forms WHERE id = ?', [id]);
      formExists = formRows.length > 0;
      ...
      if (formExists) {
        submissions = await fileStorage.getSubmissionsByFormId(id);
      }
    } else {
      formExists = inMemoryStorage.forms.some(form => form.id === id);
      
      if (formExists) {
        submissions = inMemoryStorage.submissions.filter(sub => sub.formId === id);
      }
    }
    ...
    res.send(`
      ...
      <html>
        ...
        <body>
          ...
          <p>Backend server is running. Available endpoints:</p>
...
const startServer = async () => {
  try {
    ...
    app.listen(serverPort, () => {
      ...
      if (dbConnected) {
        console.log('Database status: Connected to MySQL');
      } else if (fileStorageEnabled) {
        console.log('Storage status: Using file-based storage');
      } else {
        console.log('Storage status: Using in-memory storage');
      }
    });
  } catch (error) {
    console.error('Error starting server:', error.message);
    console.log('To kill a process using port 3001, try one of these commands:');
